...
fn stat_line(line: &str) -> Option<String> {
    ...
    let path = left.trim();
    if path.is_empty() || path.contains(' ') && !path.contains("=>") && !left.starts_with(' ') {
        return None;
    }
    ...
    if it.next().is_some() || bar.chars().any(|c| c != '+' && c != '-') {
        return None;
    }
...
fn stat_summary(line: &str) -> Option<String> {
    ...
    n.parse::<usize>().ok()?;
    let noun = hw.next()?;
    if !noun.starts_with("file") || hw.next().is_some() {
        return None;
    }
    let mut ch = Vec::new();
...
impl<'a> Hunk<'a> {
    ...
    fn wants(&mut self, line: &str) -> bool {
        ...
        match line.chars().next() {
            Some(' ') | None => {
                self.old = Some(o.saturating_sub(1));
                self.new = Some(n.saturating_sub(1));
                true
            }
            Some('-') if o > 0 => {
                self.old = Some(o - 1);
...
fn render_hunk(
    ...
    let (nums, fnctx) = match h
        .header
        .strip_prefix("@@")
        .and_then(|r| r.find("@@").map(|i| (i, r)))
    {
    ...
    out.push(if keep_fn {
        format!("{} {}", nums.trim_end(), fnctx)
    } else {
        nums.trim_end().to_string()
    });
    if !fnctx.is_empty() {
        *prev_fn = fnctx.to_string()
    }
...
fn compact_diff_hunks(lines: &[&str]) -> String {
    ...
    macro_rules! flush_file {
        () => {
            ...
            if let Some(fh) = hdr.take() {
                ...
                if dropped > 0 {
                    out.push(more(dropped, "context lines"))
                }
            }
    ...
    while i < lines.len() {
        ...
        let at_boundary = hunk.as_ref().is_none_or(Hunk::exhausted);
        if line.starts_with("@@") {
            flush_hunk!();
        ...
        if at_boundary
            && line.starts_with("--- ")
            && lines.get(i).is_some_and(|n| n.starts_with("+++ "))
            && (hdr.is_none() || emitted)
        {
            flush_file!();
            hdr = Some(FileHdr {
                a: line[4..].to_string(),
                b: lines[i][4..].trim_end_matches('\r').to_string(),
                ..Default::default()
            });
            i += 1;
            continue;
        ...
        if let Some(h) = hdr.as_mut() {
            if !emitted {
                ...
                if let Some(m) = line.strip_prefix("old mode ") {
                    h.old_mode = Some(m.to_string());
                    continue;
                }
                if let Some(m) = line.strip_prefix("new mode ") {
                    h.new_mode = Some(m.to_string());
                    continue;
                }
                if let Some(f) = line
        ...
        if hdr.is_none() {
            if let Some(s) = stat_summary(line).or_else(|| stat_line(line)) {
                out.push(s);
                continue;
            }
        }
...
fn upstream_note(t: &str) -> Option<String> {
    let up = quoted(t)?;
    if t.starts_with("Your branch is up to date with") {
        return Some(up.to_string());
    }
    if t.starts_with("Your branch is ahead of") {
        return Some(format!("{} +{}", up, first_number(&t[t.find(" by ")?..])?));
    }
    if t.starts_with("Your branch is behind") {
        return Some(format!("{} -{}", up, first_number(&t[t.find(" by ")?..])?));
    }
    if t.starts_with("Your branch is based on") && t.contains("gone") {
        return Some(format!("{} (gone)", up));
    }
    if t.starts_with("Your branch and") && t.contains("diverged") {
        return Some(up.to_string());
    }
    None
}
...
impl StatusModel {
    ...
    fn render(self) -> String {
        ...
        let empty = self.tracked.is_empty()
...
fn status_long(lines: &[&str]) -> String {
    ...
    for &line in lines {
        ...
        if t.starts_with("HEAD detached") || t.starts_with("Not currently on any branch") {
            branch = t.trim_end_matches('.').to_string();
            continue;
        }
        if t == "No commits yet" {
            no_commits = true;
        ...
        if t.starts_with("nothing to commit") {
            m.clean = true;
            continue;
        }
        if t.starts_with("nothing added to commit") || t.starts_with("no changes added to commit") {
            continue;
        }
        match t {
    ...
    for &line in lines {
        ...
        match code {
            ...
            "UU" | "AA" | "DD" | "AU" | "UA" | "DU" | "UD" => {
                m.conflicts.push(format!("{} {}", code, path))
            }
            _ => {
...
fn looks_porcelain(lines: &[&str]) -> bool {
    ...
    for l in lines.iter().filter(|l| !l.trim().is_empty()) {
        ...
        if b.len() < 4
            || b[2] != b' '
            || !b" MADRCUT?!".contains(&b[0])
            || !b" MADRCUT?!".contains(&b[1])
        {
            return false;
        }
        any = true;
...
pub(crate) fn filter_git_status(output: &str) -> String {
    ...
    let first = lines
        ...
        .copied()
        .unwrap_or("");
    if looks_porcelain(&lines) {
        return status_short(&lines);
    }
...
pub(crate) fn filter_git_diff(output: &str) -> String {
    ...
    if looks_like_diff(&lines) {
        return compact_diff(&lines);
    }
    if lines
        .iter()
        .any(|l| stat_line(l).is_some() || stat_summary(l).is_some())
    {
        return compact_stat_block(&lines);
    }
...
fn commit_header(line: &str) -> Option<(&str, &str, &str)> {
    ...
    if !(prefix.is_empty()
        || (prefix.contains('*') && prefix.chars().all(|c| GRAPH_CHARS.contains(c))))
    {
        return None;
    }
    let rest = &line[idx + 7..];
    let (sha, decor) = rest.split_once(' ').unwrap_or((rest, ""));
    if sha.len() < 7 || !is_hex(sha) {
        return None;
    }
    Some((prefix, sha, decor.trim()))
}

fn strip_graph(line: &str, w: usize) -> &str {
    if w == 0 {
        return line;
    }
    let ok = line.len() >= w
        && line.is_char_boundary(w)
        && line[..w].chars().all(|c| GRAPH_CHARS.contains(c));
...
fn parse_log(output: &str) -> Option<(Vec<String>, Vec<Commit<'_>>)> {
    ...
    for raw in output.lines() {
        ...
        if let Some((prefix, sha, decor)) = commit_header(line) {
            commits.push(Commit {
                graph: prefix.to_string(),
                sha: sha.to_string(),
                decor: decor.to_string(),
                merge: Vec::new(),
                author: String::new(),
                date: String::new(),
                msg: Vec::new(),
        ...
        } else if let Some(r) = content
            .strip_prefix("Date: ")
            .or_else(|| content.strip_prefix("AuthorDate: "))
        {
            c.date = short_date(r);
...
fn render_log(preamble: Vec<String>, commits: Vec<Commit>) -> String {
    ...
    let same_author = commits.len() > 1
        && !commits[0].author.is_empty()
        && commits.iter().all(|c| c.author == commits[0].author);
    if same_author {
        out.push(format!("author: {}", commits[0].author))
    }
    ...
    for c in commits.into_iter().take(l.list_max_lines) {
        ...
        if let Some(subject) = c.msg.iter().find(|m| !m.trim().is_empty()) {
            h.push(' ');
            h.push_str(subject.trim())
        }
        if !c.date.is_empty() {
            h.push_str(&format!(" ({})", c.date))
        }
        if !same_author && !c.author.is_empty() {
            h.push_str(&format!(" <{}>", c.author))
        }
        if !c.merge.is_empty() {
            h.push_str(&format!(" (merge {})", c.merge.join(" ")))
        }
        ...
        if body.len() > 1 {
            h.push(' ');
            h.push_str(&more(body.len(), "body lines"))
        }
        out.push(h);
        if body.len() == 1 {
            out.push(format!("  {}", body[0].trim()))
        }
        out.extend(c.stats.iter().map(|s| format!(" {}", s)));
        out.extend(c.extra.iter().map(|s| format!(" {}", s)));
        if !c.diff.is_empty() {
            out.push(compact_diff(&c.diff))
        }
    }
    if total > l.list_max_lines {
        out.push(more(total - l.list_max_lines, "commits"))
    }
    out.join("\n")
}
...
pub(crate) fn filter_git_log(output: &str) -> String {
    if let Some((pre, commits)) = parse_log(output) {
        return render_log(pre, commits);
    }
    let lines: Vec<&str> = output.lines().collect();
    if looks_oneline(&lines) {
        return cap_lines(
            lines.iter().map(|l| l.trim_end()).filter(|l| !l.is_empty()),
...
pub(crate) fn filter_git_branch(output: &str) -> String {
    ...
    for line in output.lines() {
        ...
        let (cur, name) = match t.strip_prefix("* ") {
            Some(r) => (true, r),
            None => (false, t.strip_prefix("+ ").unwrap_or(t)),
        };
        let name = squeeze_ws(name);
        if let Some((remote, rest)) = name
    ...
    for (remote, names) in remotes {
        out.push(format!(
            "remotes/{}: {}",
            remote,
            cap_vec(names, l.list_max_lines, "branches").join(", ")
        ));
    }
    out.join("\n")
}
...
pub(crate) fn filter_git_remote(output: &str) -> String {
    ...
    for line in output.lines() {
        ...
        if p.len() == 3 && matches!(p[2], "(fetch)" | "(push)") {
            ...
            other.push(if line.starts_with(char::is_whitespace) {
                format!(" {}", p.join(" "))
            } else {
                p.join(" ")
            });
...
fn filter_git_blame(output: &str) -> String {
    ...
    for line in output.lines() {
        ...
        if !is_hex(sha.trim_start_matches('^')) {
            return generic(output);
        }
        let Some(lp) = rest.find('(') else {
            return generic(output);
        };
        let Some(rp) = rest[lp..].find(')').map(|i| i + lp) else {
            return generic(output);
        };
        ...
        let content = rest
            ...
            .unwrap_or("");
        let Some((lineno, meta)) = meta.split_last() else {
            return generic(output);
        };
        // author = tokens before the date; date only (time/tz dropped)
        let mut header = if sha.len() > 8 {
            format!(
                "{}{}",
                if sha.starts_with('^') { "^" } else { "" },
                sha7(sha.trim_start_matches('^'))
            )
        } else {
            sha.to_string()
        };
    ...
    let mut cut = 0usize;
    for (h, v) in runs {
        if shown >= l.passthrough_max_lines {
            cut += v.len();
            continue;
        }
        out.push(h);
...
fn is_progress(t: &str) -> bool {
    ...
    PROGRESS.iter().any(|p| t.starts_with(p)) && !t.starts_with("Total ")
        || t.starts_with("Total ") && t.contains("delta")
}
...
fn compact_result(output: &str) -> String {
    ...
    for raw in output.split(['\n', '\r']) {
        ...
        let t = line.trim();
        if t.is_empty() || is_progress(t) || t.starts_with('(') && t.contains("git ") {
            continue;
        }
        if DETACHED_ADVICE.iter().any(|a| t.contains(a)) {
            continue;
        }
        if let Some(h) = t.strip_prefix("hint: ") {
            if !h.trim().is_empty() {
        ...
        if let Some(n) = upstream_note(t) {
            match out.last_mut() {
                Some(prev) if prev.starts_with("Switched") || prev.starts_with("Already on") => {
                    prev.push_str(&format!(" [{}]", n))
                }
                _ => out.push(format!("...{}", n)),
            }
            continue;
        ...
        if diverged && t.starts_with("and have ") {
            let nums: Vec<usize> = t
                ...
                .filter_map(|w| w.parse().ok())
                .collect();
            if let (Some(prev), Some(a), Some(b)) = (out.last_mut(), nums.first(), nums.get(1)) {
                prev.push_str(&format!(" +{} -{}", a, b))
            }
            continue;
        }
        ...
        if let Some(s) = stat_line(line) {
            out.push(format!(" {}", s));
            continue;
        }
        if let Some(p) = t.strip_prefix("create mode ") {
            created.push(p.split_once(' ').map_or(p, |x| x.1).to_string());
            continue;
        }
        if let Some(p) = t.strip_prefix("delete mode ") {
            deleted.push(p.split_once(' ').map_or(p, |x| x.1).to_string());
            continue;
        }
        if let Some(p) = t
            .strip_prefix("rename ")
            .or_else(|| t.strip_prefix("copy "))
        {
            renamed.push(p.replace(" => ", " → "));
            continue;
        }
        if t.starts_with("warning: in the working copy of '") && t.contains("will be replaced") {
            if let Some(p) = quoted(t) {
                crlf.push(p.to_string())
            }
            continue;
        }
        if t.starts_with("warning: LF will be replaced")
            || t.starts_with("warning: CRLF will be replaced")
        {
            crlf.push(t.trim_start_matches("warning: ").to_string());
            continue;
        }
        if t.starts_with("The file will have its original line endings") {
            continue;
        }
    ...
    if !created.is_empty() {
        out.push(format!(
            "created: {}",
            cap_vec(created, l.status_max_files, "files").join(", ")
        ))
    }
    if !deleted.is_empty() {
        out.push(format!(
            "deleted: {}",
            cap_vec(deleted, l.status_max_files, "files").join(", ")
        ))
    }
    if !renamed.is_empty() {
        out.push(format!(
            "renamed: {}",
            cap_vec(renamed, l.status_max_files, "files").join(", ")
        ))
    }
    if !crlf.is_empty() {
        out.push(format!(
            "warning: line endings will be replaced: {}",
            cap_vec(crlf, l.status_max_files, "files").join(", ")
        ))
    }
    if let Some(first) = hints.first() {
        ...
        if hints.len() > 1 {
            h.push(' ');
            h.push_str(&more(hints.len() - 1, "hint lines"))
        }
        out.push(h);
    }
...
fn filter_git_result(output: &str) -> String {
    if output
        .lines()
        .any(|l| l.starts_with("On branch ") || l.starts_with("HEAD detached"))
    {
...
pub(crate) fn filter_git(args: &[&str], output: &str) -> String {
    ...
    match sub {
        ...
        Some("diff" | "difftool" | "range-diff" | "format-patch" | "apply") => {
            filter_git_diff(output)
        }
        ...
        Some(
            ...
            | "revert" | "reset" | "restore" | "rm" | "mv" | "init" | "am" | "bisect" | "clean"
            | "gc" | "prune",
        ) => filter_git_result(output),
        Some("tag") => compact_list(output, "tags"),
        Some("worktree") => compact_list(output, "worktrees"),
        Some("shortlog") => compact_list(output, "lines"),
        Some(
            "ls-files" | "ls-tree" | "ls-remote" | "rev-list" | "rev-parse" | "for-each-ref"
            | "config" | "describe" | "grep" | "show-ref" | "cat-file" | "count-objects"
            | "name-rev",
        ) => compact_list(output, "lines"),
        _ => generic(output),
    }
...
fn is_header_row(cells: &[String]) -> bool {
    cells.len() > 1
        && cells.iter().all(|c| {
            !c.is_empty()
                && c.chars()
                    .all(|ch| ch.is_ascii_uppercase() || ch == ' ' || ch == '_' || ch == '#')
        })
}
...
fn table_rows(output: &str, what: &str, checks: bool) -> String {
    ...
    for line in output.lines() {
        ...
        if checks
            && cells
                .get(1)
                .is_some_and(|s| matches!(s.as_str(), "pass" | "success" | "skipping"))
        {
...
pub(crate) fn filter_gh(args: &[&str], output: &str) -> String {
    compact_json_output(output, |o| {
        match (args.first().copied(), args.get(1).copied()) {
            ...
            (
                Some(
                    "pr" | "issue" | "run" | "release" | "repo" | "gist" | "workflow" | "label"
                    | "search",
                ),
                Some("list" | "ls" | "repos" | "issues" | "prs"),
            ) => table_rows(o, "rows", false),
            (
                Some("pr" | "issue" | "release" | "run" | "repo" | "gist" | "workflow"),
                Some("view"),
            ) => kv_view(o),
...
pub(crate) fn filter_glab(args: &[&str], output: &str) -> String {
    compact_json_output(output, |o| {
        match (args.first().copied(), args.get(1).copied()) {
            (Some("api"), _) => generic(o),
            (
                Some("mr" | "issue" | "ci" | "pipeline" | "release" | "repo"),
                Some("list" | "ls"),
            ) => table_rows(o, "rows", false),
...
fn filter_jj_log(output: &str) -> String {
    ...
    for line in output.lines() {
        ...
        let is_header = toks
            .iter()
            .enumerate()
            .any(|(i, t)| is_iso_date(t) && toks.get(i + 1).is_some_and(|n| n.contains(':')));
        if is_header {
            ...
            rows.push(format!(
                "{}{}",
                sym.map(|s| format!("{} ", s)).unwrap_or_default(),
                h.join(" ")
            ));
            open_header = true;
        } else if open_header {
            if let Some(last) = rows.last_mut() {
                last.push_str(": ");
                last.push_str(&squeeze_ws(content))
            }
            open_header = false;
        } else {
...
pub(crate) fn filter_jj(args: &[&str], output: &str) -> String {
    match find_subcommand(args) {
        ...
        Some("status" | "st" | "bookmark" | "branch" | "git" | "workspace" | "files") => {
            compact_list(output, "lines")
        }
        _ => generic(output),
    }
...
mod tests {
    ...
    fn log_default_format_becomes_one_line_per_commit() {
        ...
        assert!(
            out.contains("fix(stability): add proxy loopback guard"),
            "{}",
            out
        );
        assert!(out.contains("365096f"), "{}", out);
        assert!(
            out.contains("feat(ports): migrate default ports"),
            "{}",
            out
        );
        // the 40-char sha, the e-mail and the Date: label are all redundant
        assert!(
            !out.contains("5b641e3240792aa59ddaa6f831c1dec86706093b"),
            "{}",
            out
        );
    ...
    fn log_body_lines_are_counted_not_dropped_silently() {
        ...
        assert!(
            has_truncation(&out) || out.contains("body"),
            "body lines vanished: {}",
            out
        );
    }
    ...
    fn gh_and_glab_and_jj_are_handled() {
        ...
        let jj = filter_jj(
            &["log"],
            "@  qpvuntsm user@host 2026-09-09 12:00:00 abc1234\n│  subject line\n",
        );
        assert!(
            jj.contains("subject line") || jj.contains("abc1234"),
            "{}",
            jj
        );
    }
