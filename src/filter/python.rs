...
fn split_diag(line: &str) -> Option<(String, String, String)> {
    ...
    let (pos, msg) = if third.trim().chars().all(|c| c.is_ascii_digit()) && !third.trim().is_empty() {
        (format!("{}:{}", l1, third.trim()), parts.next().unwrap_or("").trim().to_string())
    } else {
...
const PYTEST_HEADER: [&str; 10] = [
    ...
    "configfile:", "testpaths:", "benchmark:",
];
...
fn pytest_summary(line: &str) -> Option<String> {
    ...
    if !["passed", "failed", "error", "skipped", "no tests ran", "xfailed", "xpassed"]
        .iter()
...
pub(crate) fn filter_pytest(_args: &[&str], output: &str) -> String {
    ...
    let bump = |per_file: &mut Vec<(String, [usize; 4])>, file: &str, slot: usize| {
    ...
    for line in output.lines() {
        ...
        if in_section.is_empty() {
            ...
            let verdict = toks
                .iter()
                .find(|t| matches!(**t, "PASSED" | "FAILED" | "SKIPPED" | "ERROR" | "XFAIL" | "XPASS"))
                .copied();
            ...
            if let (Some(v), Some(id)) = (verdict, id) {
                ...
                if slot == 1 && !fails.iter().any(|f| f.name == id) {
                    ...
                    fails.push(Fail { name: id.to_string(), lines: Vec::new() });
                }
                continue;
            }
        }
        ...
        if tt.contains(".py ")
            && tt.ends_with(']')
            && tt.split_whitespace().nth(1).map(|p| p.chars().all(|c| ".FEsxX".contains(c))).unwrap_or(false)
        {
            continue;
        }
        match in_section {
            "failures" | "errors" => {
                ...
                if tt.starts_with('_') && tt.ends_with('_') {
                    ...
                    if !name.is_empty() {
                        fails.push(Fail { name: name.to_string(), lines: Vec::new() });
                    }
                    continue;
                }
                if let Some(f) = fails.last_mut() {
                    ...
                    let is_frame = tt.starts_with("File \"")
                        || (tt.contains(".py:")
                            && tt.split(':').nth(1).map(|n| n.trim().chars().all(|c| c.is_ascii_digit())).unwrap_or(false));
    ...
    if per_file.len() > 1 {
        for (file, c) in &per_file {
            ...
            if c[1] > 0 { bits.push(format!("{} failed", c[1])) }
            if c[2] > 0 { bits.push(format!("{} skipped", c[2])) }
            if c[3] > 0 { bits.push(format!("{} error", c[3])) }
...
pub(crate) fn filter_mypy(output: &str) -> String {
    group_diags("mypy", output, |t| t.starts_with("Found ") || t.starts_with("Success:"))
}
...
pub(crate) fn filter_ruff(output: &str) -> String {
    group_diags("ruff", output, |t| t.starts_with("Found ") || t.starts_with("All checks passed"))
}
...
fn format_tool(tool: &str, output: &str) -> String {
    ...
    for line in output.lines() {
        ...
        if let Some(p) = t.strip_prefix("would reformat ").or_else(|| t.strip_prefix("reformatted ")) {
        ...
        if t.contains("left unchanged") || t.contains("files would be left unchanged") || t.starts_with("Skipped") {
            if let Some(n) = t.split_whitespace().next().and_then(|n| n.parse::<usize>().ok()) {
                unchanged += n;
            }
            continue;
        }
    ...
    let mut out = Vec::new();
    if !changed.is_empty() {
        out.push(format!("{}: {} to reformat:", tool, plural(changed.len(), "file")));
...
const PIP_NOISE: [&str; 8] = [
    ...
    "Preparing metadata", "Building wheel", "Created wheel", "Stored in directory",
];
    ...
    match find_subcommand(args) {
        ...
        Some("show") => {
            let keep: Vec<String> = output
                .lines()
                .map(|l| l.trim())
                .filter(|t| !t.is_empty() && t.split_once(':').map(|(_, v)| !v.trim().is_empty()).unwrap_or(true))
                .map(String::from)
                .collect();
...
pub(crate) fn filter_uv(args: &[&str], output: &str) -> String {
    ...
    match find_subcommand(args) {
        Some("pip") => {
            let rest: Vec<&str> = args.iter().copied().skip_while(|a| *a != "pip").skip(1).collect();
            filter_pip(&rest, output)
        }
        Some("sync") | Some("add") | Some("remove") | Some("install") | Some("lock") | Some("venv") => {
            let mut keep: Vec<String> = Vec::new();
            ...
            for line in output.lines() {
                ...
                if t.starts_with("Resolved ") || t.starts_with("Prepared ") || t.starts_with("Downloading")
                    || t.starts_with("Building ") || t.starts_with("Built ") || t.starts_with("Audited ")
                {
                    progress += 1;
                    continue;
                }
    ...
    match find_subcommand(args) {
        Some("install") | Some("add") | Some("remove") | Some("update") | Some("lock") => {
            ...
            for line in output.lines() {
                ...
                if t.starts_with("Installing ") || t.starts_with("Updating ") || t.starts_with("Removing ")
                ...
                if t.starts_with("Resolving dependencies") || t.starts_with("Writing lock file")
                    || t.starts_with("Package operations") || t.starts_with("Creating virtualenv")
                {
                    continue;
                }
                keep.push(t);
        ...
        Some("run") | Some("build") | Some("publish") => {
            cap_lines(collapse_blank(output).lines(), l.passthrough_max_lines, "lines")
        }
...
=================== 3 failed, 9 passed, 1 skipped in 0.12s ====================";
    ...
    fn pytest_keeps_summary_and_assertions_drops_header() {
        ...
        assert!(out.starts_with("pytest: 3 failed, 9 passed, 1 skipped in 0.12s"), "{}", out);
        ...
        assert!(out.contains("AssertionError: assert 'PRISM' == 'PRISN'"), "{}", out);
...
=================== 1 failed, 2 passed, 1 skipped in 0.05s ====================";
    ...
    fn pytest_failure_cap_announced_and_errors_kept() {
        ...
        assert!(out.contains("ERROR test_y.py - ImportError: boom"), "{}", out);
    ...
    fn mypy_ruff_group_by_file_and_report_clean() {
        ...
        assert!(out.contains("app/main.py\n  12  error: Incompatible return value type"), "{}", out);
        assert!(out.contains("app/db.py\n  3  error: Cannot find implementation"), "{}", out);
    ...
    fn ruff_diagnostics_have_col_and_code() {
        let out = filter_ruff("src/a.py:1:1: F401 [*] `os` imported but unused\nsrc/a.py:9:5: E711 comparison to None\nFound 2 errors.");
        assert!(out.contains("src/a.py\n  1:1  F401 [*] `os` imported but unused"), "{}", out);
    ...
    fn black_lists_files_and_reports_clean() {
        let out = filter_black("would reformat src/a.py\nwould reformat src/b.py\nOh no! 💥 💔 💥\n2 files would be reformatted, 8 files would be left unchanged.");
        assert!(out.contains("black: 2 files to reformat:"), "{}", out);
        assert!(out.contains("src/a.py") && out.contains("src/b.py"), "{}", out);
        assert!(out.contains("Oh no!"), "{}", out);
        assert_eq!(filter_black("12 files would be left unchanged."), "black: ok (12 unchanged)");
    }

    #[test]
    fn pip_list_and_install_compact() {
        let list = filter_pip(&["list"], "Package    Version\n---------- -------\nrequests   2.31.0\nurllib3    2.2.1");
        ...
        assert!(inst.contains("Successfully installed requests-2.31.0"), "{}", inst);
    ...
    fn bandit_keeps_issues_and_totals() {
        let out = filter_bandit(">> Issue: [B101:assert_used] Use of assert detected.\n   Severity: Low   Confidence: High\n   Location: app/x.py:12:4\nTotal issues (by severity):\n\tHigh: 0");
